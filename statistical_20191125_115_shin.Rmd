...
![](https://rafalab.github.io/dsbook/book_files/figure-html/first-confidence-intervals-example-1.png)
...
We want to know the probability that the interval $[\bar{X}-2\hat{SE}(\bar{X}),(\bar{X}−2\hat{SE}(\bar{X})]$ contains the true proportion $p$. First, consider that the start and end of these intervals are random variables: every time we take a sample, they change. To illustrate this, run the Monte Carlo simulation above twice. We use the same parameters as above:
```{r}
p <- 0.45
N <- 1000
```
...
To determine the probability that the interval includes $p$, we need to compute this:$$Pr(\bar{X}−1.96\hat{SE}(\bar{X})≤p≤\bar{X}+1.96\hat{SE}(\bar{X}))$$
By subtracting and dividing the same quantities in all parts of the equation, we get that the above is equivalent to:$$Pr(−1.96≤\frac{\bar{X}−p}{\hat{SE}(\bar{X})}≤1.96)$$
The term in the middle is an approximately normal random variable with expected value 0 and standard error 1, which we have been denoting with $Z$, so we have:$$Pr(−1.96≤Z≤1.96)$$
...
If we want to have a larger probability, say 99%, we need to multiply by whatever `z` satisfies the following:$$Pr(−z≤Z≤z)=0.99$$
Using:
```{r}
z <- qnorm(0.995)
z
...
![](https://rafalab.github.io/dsbook/book_files/figure-html/confidence-interval-coverage-1.png)
...
```{r}
poll <- polls %>%
  mutate(lower = x_hat-1.96*se_hat, upper = x_hat+1.96*se_hat, x_hat = polls$rawpoll_clinton[1]/100, se_hat = sqrt(x_hat*(1-x_hat)/N))
select(poll, pollster, enddate, x_hat, lower, upper)
```
...
poll2 <- poll2 %>% mutate(lower = x_hat-1.96*se_hat, upper = x_hat+1.96*se_hat, se_hat = sqrt(x_hat*(1-x_hat)/N))
poll2 <- poll2 %>% mutate(hit = ifelse(lower<=0.482 & upper>=0.482, 1, 0))
```
...
polls %>%
  group_by(pollster) %>%
  ggplot(aes(str_wrap(pollster, 10), error)) + geom_boxplot() + theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
A 2014 PNAS paper^[52](https://rafalab.github.io/dsbook/inference.html#fn52)^ analyzed success rates from funding agencies in the Netherlands and concluded that their:
...
R.A. Fisher^[53](https://rafalab.github.io/dsbook/inference.html#fn53)^ was one of the first to formalize hypothesis testing. The “Lady Tasting Tea” is one of the most famous examples.
...
An informative summary statistic associated with two-by-two tables is the odds ratio. Define the two variables as $X=1$ if you are a male and 0 otherwise, and $Y=1$ if you are funded and 0 otherwise. The odds of getting funded if you are a man is defined:$$Pr(Y=1∣X=1)/Pr(Y=0∣X=1)$$ and can be computed like this:
```{r}
odds_men <- with(two_by_two, (men[2]/sum(men)) / (men[1]/sum(men)))
odds_men
```
And the odds of being funded if you are a woman is:$$Pr(Y=1∣X=0)/Pr(Y=0∣X=0)$$ and can be computed like this:
```{r}
...
This implies that a 95% confidence interval for the log odds ratio can be formed by:$$log(\frac{ad}{bc})±1.96\sqrt{1/a+1/b+1/c+1/d}$$
...
```{r}
two_by_two <- data.frame(record = c("wins", "loses"),
                         matches = c(500*0.7, 500*0.3),
                         event = c(8, 9))
...
50. http://www.realclearpolitics.com[↩](https://rafalab.github.io/dsbook/inference.html#fnref50)

51. http://www.realclearpolitics.com/epolls/2016/president/us/general_election_trump_vs_clinton-5491.html[↩](https://rafalab.github.io/dsbook/inference.html#fnref51)

52. http://www.pnas.org/content/112/40/12349.abstract[↩](https://rafalab.github.io/dsbook/inference.html#fnref52)

53. https://en.wikipedia.org/wiki/Ronald_Fisher[↩](https://rafalab.github.io/dsbook/inference.html#fnref53)